...
class GradAscent(UnlearnMethod):
    ...
    def get_unlearned_model(self) -> nn.Module:
        ...
        if self.sched == "cosine":
            ...
            scheduler = LambdaLR(
                optimizer,
                lr_lambda=lambda step: linear_schedule(
                    step, self.epochs * len(forget_trainloader)
                ),
            )
        else:
            scheduler = None
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            print(
                f"Train Epoch {epoch} Loss {losses.avg:.4f} Acc {top1.avg:.4f} LR {lr} Time: {finish-start:.2f}s"
            )
